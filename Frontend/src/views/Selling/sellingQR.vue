<template>
    <div class="modal fade" tabindex="-1" role="dialog"  id="sellingQR"> <!-- ref="modal" -->
                ...
                <div class="modal-body">
                    <div class="form-group col" v-show="qrImage.svg">
                        ...
                        <div class="row text-center">
                            <div class="col-6"><label>ชื่อร้าน : {{detail.or_customer_name}}</label></div>
                            <div class="col-6"><label>จำนวนเงิน : {{detail.or_pay}}</label></div>
                        </div>
                    </div>
...
export default {
    ...
    watch: {
        code(value){
            ...
            axios.get(`/api/userinfo/${value.or_customer_id}`)
                .then(res => this.store = res.data ).catch(error => this.alertify.error(error.response.data.message))
        },
        store(value){
            axios.post(`/api/qrcode/store/${this.code.or_pay}`, this.store)
        ...
        Image(value){
            ...
            this.code.or_imageRepay = value
            this.code.or_Repay = this.code.or_pay
        }
    },
    computed: {
    ...
    mounted() {
        ...
        this.jquery("#sellingQR").on("hide.bs.modal", event => {
    ...
    methods: {
        submit(){
            ...
            axios.post(`/api/customer/order/${this.code.or_id}`, this.code).then(res => {
                    this.jquery('#sellingQR').modal('hide');
