...
export default {
    ...
    watch: {
        message(value){
            if(!value) return
            this.jquery("#message-dialog").modal();
            ...
            if (value.id) {
                ...
                this.send.rec_id = value.rec_id
                this.send.rec_name = value.rec_name
                this.send.send_id = value.send_id
                this.send.send_name = value.send_name

            } else if (value.u_id) { // ติดตาม admin
                this.youId = 1
                this.getMessage();
                
                this.send.rec_id = 1
                this.send.rec_name = 'admin'
                this.send.send_id = value.u_id
                this.send.send_name = value.u_fullname
            }
        ...
        chat(num){
            ...
            num.send_id == this.userId.u_id ? this.youId = num.rec_id : this.youId = num.send_id;
    ...
    mounted() {
        ...
        this.jquery("#message-dialog").on("hide.bs.modal", event => {
