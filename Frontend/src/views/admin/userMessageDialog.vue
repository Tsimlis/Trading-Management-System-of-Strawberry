<template>
    <div class="modal fade" tabindex="-1" role="dialog" id="chat-dialog">
        <div class="modal-dialog" role="document">
            <form class="modal-content" @submit.prevent="onSubmit()">
                    ...
                    <div v-if="chat.send_id == userId.u_id">
                        <div class="form-group col mb-0" v-for="item in data" :key="item" >
                            <div  v-if="JSON.parse(item).receive">
                                ...
                                <div class="row pl-3" >
                                    <label class="p-1 mb-2 bg-warning text-dark shadow " style="border-radius: 10px;">{{ JSON.parse(item).receive }}</label>
                                </div>
                            </div>
                            ...
                            <div class="form-group text-right m-0" @click="onDelete(item)" v-if="JSON.parse(item).send">
                                ...
                                <label class="p-1 mb-2 bg-secondary text-white shadow " style="border-radius: 10px;">{{ JSON.parse(item).send }}</label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="chat.send_id != userId.u_id">
                        <div class="form-group col mb-0" v-for="item in data" :key="item" >
                            <div  v-if="JSON.parse(item).send">
                                ...
                                <div class="row pl-3" >
                                    <label class="p-1 mb-2 bg-warning text-dark shadow " style="border-radius: 10px;">{{ JSON.parse(item).send }}</label>
                                </div>
                            </div>
                            
                            <div class="form-group text-right m-0" @click="onDelete(item)" v-if="JSON.parse(item).receive">
                                <label class="m-0 pt-2 align-text-bottom" style="font-size: 10px;"> {{ JSON.parse(item).time }} </label><br>
                                <label class="p-1 mb-2 bg-secondary text-white shadow " style="border-radius: 10px;">{{ JSON.parse(item).receive }}</label>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-footer">
                    <div class="col">
                        <input type="text" class="form-control " placeholder="เขียนข้อความ" v-model="send.message">
...
export default {
    ...
    watch: {
        message(value){
            if(!value) return
            this.jquery("#chat-dialog").modal();
            this.youId = value.u_id

            //แสดงข้อความของทั้ง 2 คน
            this.getMessage(); 
            setInterval(() => { this.getMessage(); }, 5000);
            
            this.send.rec_id = value.u_id
            this.send.rec_name = value.u_fullname
    ...
    mounted() {
        ...
        this.jquery("#chat-dialog").on("hide.bs.modal", event => {
